  ...
  return (
    <div className="page-card p-4 sm:p-5">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-lg font-semibold text-brand-navy dark:text-white">Activity Logs</h2>
        <span className="badge">{logs.length} recent</span>
      </div>
      <div className="mt-4 max-h-[420px] space-y-3 overflow-y-auto pr-1">
        {logs.length === 0 ? (
          <div className="rounded-lg border border-dashed border-slate-300 bg-slate-50/70 p-6 text-center text-sm text-slate-500 dark:border-slate-700 dark:bg-slate-950/50 dark:text-slate-400">
            No activity yet.
          </div>
        ) : (
          logs.map((log) => (
